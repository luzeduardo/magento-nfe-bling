...
class Lema21_Nfe_Model_TransformToXML
{
    ...
    private function _addItens()
    {
        ...
        $cont=0;
        foreach ($this->_orderModel->getAllItems() as $itemId => $item) {
            Mage::log(" origem > " . 
              $item->getProduct()->getResource()->getAttribute('codigo_origem')->getFrontend()->getValue($item->getProduct())
            );
            Mage::log(" origem 2 > " . $item->getProduct()->getData("codigo_origem"));
            $this->_xml["itens"]["item"][$cont] = array(
                "codigo" => $item->getSku(),
                "descricao" => $item->getProduct()->getData("operation_name"),
                "un"    => $item->getProduct()->getData("operation_unit"),
                ...
                "class_fiscal" => $item->getProduct()->getData("codigo_ncm"),
                "origem"    => $item->getProduct()->getData("codigo_origem")
                );
